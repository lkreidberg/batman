...
double intensity(double r, double c1, double c2, double c3, double c4, double norm)
{
...
static PyObject *_nonlinear_ld(PyObject *self, PyObject *args)
{
	...
	int nthreads;
	npy_intp i, dims[1];
	double dr, A_f, r_in, r_out, delta, c1, c2, c3, c4;
	
	PyArrayObject *zs, *flux;
  	if(!PyArg_ParseTuple(args,"Oddddddi", &zs, &rprs, &c1, &c2, &c3, &c4, &fac, &nthreads)) return NULL;
	...
	for(i = 0; i < dims[0]; i++)
	{
		...
		else
		{
			...
			while(r < r_out)
			{
				...
				I = intensity(r - dr/2.,c1,c2, c3, c4, norm); 	//intensity at the midpoint
